...
| **Database / Supabase** | Postgres schema design, migrations, indexes, transactions, `pg_cron`, Storage | Single source of truth for catalog, orders, inventory, users | Supabase, Postgres 15+, SQL |
...
| **Barcode scanning** | Camera-based UPC/EAN scanning, fast lookup, multi-barcode handling, manual fallback | Customer lookup + admin stock/price tools | `expo-camera` `CameraView` |
| **Inventory management** | Stock reservation/decrement/restore, low-stock alerts, audit logging, status rules | Prevents overselling; keeps the catalog honest | Postgres transactions, inventory logs |
...
| **Backend / Edge Functions** | Server-side TypeScript, secure use of service role, owner-checked operations, webhooks | Privileged work that must never touch the client | Supabase Edge Functions (Deno) |
...
- 1 full-stack developer comfortable with **Expo + Supabase + Postgres RLS** can build the MVP.
