...
6a. **Clover scope = payments-only for the MVP (approach A), schema-ready for B.** Supabase stays the order/app system of record. If the store already manages products in Clover, enable a **one-way Clover→Supabase** catalog/stock sync (`system_settings.clover_sync_mode`); never write stock back at MVP. Clover-as-inventory/order-system-of-record (B) is a deliberate future upgrade. Persist all Clover IDs now so B is additive. Supabase RLS remains the source of truth for **app permissions**; Clover governs money (and optionally catalog), never who-can-do-what.
...
- **Stack:** Supabase (Postgres + Auth + RLS + Storage + Edge Functions), React Native/Expo (current stable SDK) customer app, React + Vite admin web, **Clover (payments — Hosted Checkout; optional one-way catalog source)**, Twilio (SMS), Resend/SendGrid (email), Sentry (monitoring).
...
- **Inventory rule:** reserve on checkout, decrement on admin accept, restore on cancel; every change logged.
